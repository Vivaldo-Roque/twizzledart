...
/// Ported to native/ — no OpenGL dependency, pure GLM math.
...
inline bool affectsCubie(int sliceSign, int coord) {
    if (sliceSign == 9)   return true;
    if (sliceSign == 10)  return coord >= 0;
    if (sliceSign == -10) return coord <= 0;
    return coord == sliceSign;
}
...
void MoveAnimator::stepForward() {
    ...
    playheadMs_ = (float)std::min(idx + 1, (int)moves_.size()) * moveDurationMs_;
}
...
glm::mat4 MoveAnimator::modelMatrix(int index) const {
    ...
    float t = std::clamp((playheadMs_ - boundaryMs) / moveDurationMs_, 0.0f, 1.0f);

    MoveParams p      = paramsForMove(moves_[moveIdx]);
    const glm::mat4& base = checkpoints_[moveIdx][index];

    glm::ivec3 grid = glm::ivec3(glm::round(glm::vec3(base[3])));
    int coord = (p.sliceAxis==0) ? grid.x : (p.sliceAxis==1) ? grid.y : grid.z;
    if (!affectsCubie(p.sliceSign, coord)) return base;

    float angle = p.totalAngle * smootherStep(t);
