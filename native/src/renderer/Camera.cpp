...
OrbitCamera::OrbitCamera()
    : latitude_(35.0f), longitude_(30.0f), radius_(6.8f),
...
void OrbitCamera::setPitchLock(bool locked) {
    pitchLock_ = locked;
    if (pitchLock_) latitude_ = std::clamp(latitude_, -89.0f, 89.0f);
}
...
void OrbitCamera::drag(float dx, float dy) {
    ...
    latitude_   = std::fmod(latitude_, 360.0f); // Allow 360 orbit
    longitude_  = std::fmod(longitude_, 360.0f);
    if (pitchLock_) latitude_ = std::clamp(latitude_, -89.0f, 89.0f);
...
void OrbitCamera::setPosition(float latitude, float longitude, float radius) {
    latitude_  = pitchLock_ ? std::clamp(latitude, -89.0f, 89.0f) : latitude;
